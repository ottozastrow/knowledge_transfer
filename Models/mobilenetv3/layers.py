...
from Models.mobilenetv3.utils import get_layer
...
class ReLU6(tf.keras.layers.Layer):
    ...
    def get_config(self):
        config = super().get_config().copy()
        config.update({'relu6': self.relu6,
            'name': self.name,
        })
        return config
...
class HardSwish(tf.keras.layers.Layer):
    ...
    def get_config(self):
        config = super().get_config().copy()
        config.update({'hard_sigmoid': self.hard_sigmoid,
            'name': self.name,
        })
        return config
...
class BatchNormalization(tf.keras.layers.Layer):
    ...
    def get_config(self):
        ...
        containing the configuration of a layer. The same layer can be reinstantiated
        later (without its trained weights) from this configuration"""
        config = super().get_config().copy()
        config.update({
            'bn': self.bn,
            'name': self.name,
...
class Bneck(tf.keras.layers.Layer):
    def __init__(
        ...
        super().__init__(name="Bneck_" + str(num))
