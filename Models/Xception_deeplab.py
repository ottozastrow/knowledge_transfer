...
def Xception(mattr, **kwargs):
    '{"dropout_rate":0.55, "OS":16, "mid_repeat":8}'
    if('dropout_rate' not in kwargs.keys()):
        dropout_rate = 0.55
    else:
        dropout_rate=kwargs['dropout_rate']
    if('OS' not in kwargs.keys()):
        OS = 16
    else:
        OS=kwargs['OS']
    if('mid_repeat' not in kwargs.keys()):
        mid_repeat = 8
    else:
        mid_repeat=kwargs['mid_repeat']
    ...
    b4 = Lambda(lambda x: K.expand_dims(x, 1))(b4)
    shape_after_gapool = tf.shape(x)
    # b4 = Reshape(( 1, 1, mattr.nclasses))(b4)
    ...
    b4 = Lambda(lambda x: tf.compat.v1.image.resize(x, 
                                                    # size_before[1:3],
                                                    [128 //16, 256 //16],
                                                    # [mattr.inheight //16, mattr.inwidth//16],
    ...
    x = Lambda(lambda xx: tf.compat.v1.image.resize(xx,
                                                        # skip1.shape[1:3],
                                                        [128 //4, 256//4],
                                                        # [mattr.inheight //4, mattr.inwidth//4],
